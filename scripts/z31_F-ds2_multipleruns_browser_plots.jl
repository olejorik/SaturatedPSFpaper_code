...
for d in alldicts
    ...
    for nrun in 1:nruns
        ...
        Label(fig[0, :], makelabel(psfname, input_params, preprocess_params, PRparams), texttsize=12)
        # fig |> display
...
# for sel_dict in sel_dicts
    ...
    rowgap!(fig.layout, 5)
    # N = sel_dict[:N]
    # Label(fig[0, :], "Noise level N = $N", texttsize =30)
...
# for sel_dict in sel_dicts
    ...
    for (j, sat) in enumerate(params_input_data[:sat])
        for (i, threshold) in enumerate(preprocess_params_all[:threshold])
            ...
            Label(fig[i,j,], "$(round(v, digits=3))", texttsize =30) 
            if j ==1
    ...
    # Label(fig[-1, :], "Noise level N = $N", texttsize =30)
