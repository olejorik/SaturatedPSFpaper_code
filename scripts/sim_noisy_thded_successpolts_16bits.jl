...
for d in alldicts
    # d = alldicts[22]
    psfname, psfimage, ap, gt, bpp = read_sim_data(d, ext =  "tif")
    @unpack hw = d
    mask = ap2mask(ap)
    ...
    # Artificial bith depth, not related to the hardware
    if bpp == 8
        psfimage = reinterpret(Gray{N0f8}, psfimage)
    elseif bpp == 12
        psfimage = reinterpret(Gray{N4f12}, psfimage)
    elseif bpp == 16
        psfimage = reinterpret(Gray{N0f16}, psfimage)
    end
...
for sel_dict in sel_dicts
    ...
    N = sel_dict[:N]
    bpp = sel_dict[:bpp]
    Label(fig[0, :], "Input PSF, noise level N = $N, bpp = $bpp", fontsize=30)
    resize_to_layout!(fig)
    display(fig)

    wsave(plotsdir(experimentname, savename("inputPSFtable",sel_dict,"pdf")), fig)

    errdict[savename(sel_dict)] = errtable
    distdict[savename(sel_dict)] = disttable
end
...
for sel_dict in sel_dicts
    ...
    N = sel_dict[:N]
    bpp = sel_dict[:bpp]
    Label(fig[0, :], "Restored PSF, noise level N = $N, bpp =$bpp", fontsize=30)
    resize_to_layout!(fig)
    display(fig)

    wsave(plotsdir(experimentname, savename("restoredPSFtable",sel_dict,"pdf")), fig)
end
...
let (mask, mask2) = (nothing, nothing)
    for sel_dict in sel_dicts
        ...
        N = sel_dict[:N]
        bpp = sel_dict[:bpp]
        Label(fig[0, :], "Restored phase, noise level N = $N, bpp =$bpp", fontsize=30)
        resize_to_layout!(fig)
        display(fig)

        wsave(plotsdir(experimentname, savename("restoredphasetable",sel_dict,"pdf")), fig)
    end
end
...
for sel_dict in sel_dicts
    ...
    # vals= errdict[N]
    # vals = Int.(errtable)
    vals = distdict[savename(sel_dict)]
    fig = Figure()
    ...
    Label(fig[-1, :], "Phase error, noise level N = $N, bpp = $bpp", fontsize=30)
    Colorbar(
        fig[end+1, :];
        limits=(cmin, cmax),
        colormap=cscheme,
    ...
    resize_to_layout!(fig)
    display(fig)
    wsave(plotsdir(experimentname, savename("successruns",sel_dict,"pdf")), fig)
end
...
for sel_dict in sel_dicts
    ...
    vals= Int.(errdict[savename(sel_dict)])
    # vals = Int.(errtable)
    # vals= disttable
    fig = Figure()
    cmin, cmax = extrema(vals)
    ...
    Label(fig[-1, :], "Number of successful runs, noise level N = $N, bpp = $bpp", fontsize=30)
    Colorbar(
        fig[end+1, :];
        limits=(cmin, cmax),
        colormap=cscheme,
    ...
    resize_to_layout!(fig)
    display(fig)
    wsave(plotsdir(experimentname, savename("ressuccessruns",sel_dict,"pdf")), fig)
end