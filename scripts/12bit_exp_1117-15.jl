#  processing of the 12 bit data and using HDR PSF as ground truth
...
function run_exp(d)
    ...
    preprocess_params_all = (
        ...
        sat_level=(65535-satoffset) / 65535, # slighty less than maximum
        low_level=5/4095,
        downsample=ds,
        offset=[4.5], 
        # bgmean = [d[:bgmean]], # use standard offset 
        PRalg= ["GSSatF"],# "GSSatP"],
        beta = 
    ...
    for d in alldicts
        ...
        refphase = Float64.(load(datadir("pro","HDR",d[:psfdir],"restPhase_av=$av.tif")))
        ...
        # nrun =1
            ...
            cm = satpaletteTwoBand(dplot[:sat]; offset = 256- round(Int,d[:sat_level]*256), nsteps = 256)
